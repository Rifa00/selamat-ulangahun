...
reset("reset1", "name-i");
reset("reset2", "pesan");
const nm = ["rifa", "sitinurlaila", "siti", "sitinur", "sitinurlailasari"];
...
document.getElementById("photo-form").addEventListener("submit", async (e) => {
  ...
  if (!base64URL) {
    if (localStorage.getItem("url") !== "false") {
      base64URL = localStorage.getItem("url");
    } else {
  ...
  addItem(parseInt(id), base64URL, title, description);

  const response = await fetch(API_URL, {
    method: "POST",
    body: JSON.stringify({ action, id, base64URL, title, description }),
  });
  console.log("data masuk ke server");
  fetchData();
  reloadData();
});

function addItem(id, base64URL, title, description) {
  if (id) {
    const index = items.findIndex((item) => item[0] === id);
    const item = [id, base64URL, title, description];
    items[index] = item;
    localStorage.setItem("img", "false");
    localStorage.setItem("items", JSON.stringify(items));
    console.log("telah diubah di storage");
  } else {
    const newID = items.length + 1;
    const newItem = [newID, base64URL, title, description];
    items.push(newItem);
    localStorage.setItem("items", JSON.stringify(items));
  }
  load.classList.remove("show");
...
//   });
//   console.log("satu data di server terhapus");
//   const index = items.filter((item) => item[0] !== id);
//   localStorage.setItem("items", JSON.stringify(index));
//   console.log("satu data di storage di hapus");
//   window.location.reload();
// }
let uRL;
function editData(id, photoUrl, title, description) {
  que("#open-form").click();
  que(".reset-form").classList.remove("hidden");
  console.log(id);
